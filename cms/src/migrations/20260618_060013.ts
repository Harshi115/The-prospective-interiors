...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_projects_sector" AS ENUM('Hospitality', 'Industrial', 'Healthcare', 'Retail', 'Residential', 'Commercial', 'Civic');
  CREATE TYPE "public"."enum_inquiries_project_type" AS ENUM('Residential', 'Commercial', 'Hospitality', 'Industrial', 'Healthcare', 'Other');
  ...
  CREATE TABLE "inquiries" (
  	...
  	"email" varchar,
  	"phone" varchar,
  	"project_type" "enum_inquiries_project_type",
  	"message" varchar,
  	"submitted_at" timestamp(3) with time zone,
  	"status" "enum_inquiries_status",
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "public"."enum_inquiries_project_type";
